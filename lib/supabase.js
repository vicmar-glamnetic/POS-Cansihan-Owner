...
export async function setProductStock(itemName, newStock, currentStock) {
  // If transitioning from unlimited, treat current as 0 for delta calculation
  const effectiveCurrent = currentStock === -1 ? 0 : currentStock;
  const delta = newStock - effectiveCurrent;
  if (delta === 0 && currentStock !== -1) return true;

  // Update menu_items.stock directly if the row exists (post-sync)
  await supabase.from('menu_items').update({ stock: newStock }).eq('name', itemName);

  // Record as a correction purchase for audit trail
  const { data: purchase, error } = await supabase
    .from('purchases')
    .insert({
      local_id: null,
      supplier: 'Admin Correction',
      note: `Stock set to ${newStock} (was ${currentStock === -1 ? 'Unlimited' : currentStock})`,
      total_cost: 0,
    })
    .select()
    .single();
...
export async function setUnlimitedStock(itemName) {
  // Update menu_items.stock directly if the row exists (post-sync)
  await supabase.from('menu_items').update({ stock: -1 }).eq('name', itemName);
  return true;
}
