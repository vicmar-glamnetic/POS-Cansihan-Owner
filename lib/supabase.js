...
export async function getProductStock() {
  ...
  // Seed from menu_items first (carries the authoritative stock value)
  for (const item of (menuData || [])) {
    map[item.name] = { name: item.name, total_in: 0, total_sold: 0, app_stock: item.stock ?? null };
  }

  // Add purchase totals — also creates entries for items not yet in menu_items
  ...
  // Add sales totals — also creates entries for items only seen in orders
  ...
  return Object.values(map)
    .map(item => {
      if (item.app_stock === -1) return { ...item, current: -1 };
      if (item.app_stock != null) return { ...item, current: item.app_stock };
      // Not yet synced from app: if item has sales but zero purchases, it's unlimited on the POS app
      if (item.total_in === 0 && item.total_sold > 0) return { ...item, current: -1 };
      return { ...item, current: Math.max(0, item.total_in - item.total_sold) };
    })
    .sort((a, b) => a.name.localeCompare(b.name));
}

// Web admin sets stock directly — authoritative, no approval needed.
// Uses update-then-insert so items not yet in menu_items still get saved.
async function upsertStock(itemName, stock, isAvailable) {
  // Update returns the affected rows; empty array means no row matched
  ...
  if (updateErr) {
    console.error('[upsertStock] update error:', updateErr);
    return false;
  }
  if (data && data.length > 0) return true; // updated an existing row

  // No row matched — insert (requires local_id to be nullable in Supabase)
  console.warn('[upsertStock] no row matched for', itemName, '— attempting insert');
  const { error: insertErr } = await supabase
    .from('menu_items')
    .insert({ name: itemName, price: 0, category: '', stock, is_available: isAvailable });
  if (insertErr) console.error('[upsertStock] insert error:', insertErr);
  return !insertErr;
}

export async function setProductStock(itemName, newStock) {
  return upsertStock(itemName, newStock, newStock !== 0);
}

export async function setUnlimitedStock(itemName) {
  return upsertStock(itemName, -1, true);
}

// Get pending stock change requests submitted from the POS app
export async function getAppStockRequests() {
  const { data } = await supabase
    .from('stock_requests')
    .select('id, item_name, stock, created_at')
    .eq('source', 'app')
    .eq('status', 'pending')
    .order('created_at', { ascending: true });
  return data || [];
...
// Approve a stock request: update Supabase stock and mark as approved
export async function approveStockRequest(requestId, itemName, newStock) {
  const [updateRes, approveRes] = await Promise.all([
  ...
  return !updateRes.error && !approveRes.error;
}

// Reject a stock request (stock stays unchanged)
export async function rejectStockRequest(requestId) {
  const { error } = await supabase.from('stock_requests')
