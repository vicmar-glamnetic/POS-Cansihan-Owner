...
export async function getProductStock() {
  ...
  const [{ data: purchased }, { data: soldData }] = await Promise.all([
  ...
  const map = {};
  const hasPurchases = new Set(); // only show items that have purchase records
  ...
  return Object.values(map)
    .filter(item => hasPurchases.has(item.name)) // hide items with no purchase records
