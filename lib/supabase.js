...
export async function getProductStock() {
  ...
  const [menuRes, purchasedRes, soldRes, deletedRes] = await Promise.all([
    supabase.from('menu_items').select('name').eq('is_available', true),
    ...
    supabase.from('activity_log').select('item_name').eq('action', 'product_deleted'),
  ]);

  const deleted = new Set((deletedRes.data || []).map(l => l.item_name));
  ...
  if (menuRes.data && menuRes.data.length > 0) {
    ...
    for (const item of menuRes.data) {
      if (!deleted.has(item.name)) map[item.name] = { name: item.name, total_in: 0, total_sold: 0 };
    }
    ...
    // Fallback: derive from order + purchase history (before menu_items table is set up)
    for (const row of (purchasedRes.data || [])) {
      const n = row.item_name;
      if (!map[n]) map[n] = { name: n, total_in: 0, total_sold: 0 };
    ...
    for (const k of Object.keys(map)) {
      if (deleted.has(k)) delete map[k];
    }
  }

  return Object.values(map)
    .map(item => ({ ...item, current: Math.max(0, item.total_in - item.total_sold) }))
