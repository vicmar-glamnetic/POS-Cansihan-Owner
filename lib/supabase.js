...
export async function getProductStock() {
  ...
  if (menuData && menuData.length > 0) {
    ...
    // stock: -1 = unlimited, >=0 = exact count from POS app, undefined = not yet synced
    for (const item of menuData) {
      // null or undefined app_stock means stock column not available yet
    ...
    // menu_items table doesn't exist — fallback to order + purchase history
    for (const row of (purchasedRes.data || [])) {
  ...
  // else: menu_items table exists but empty — show nothing (waiting for first sync)
