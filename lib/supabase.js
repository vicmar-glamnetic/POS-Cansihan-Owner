...
// If the item doesn't exist in menu_items yet, insert it so the value persists.
export async function setProductStock(itemName, newStock) {
  ...
  if (existing) {
    ...
    const { error } = await supabase.from('menu_items')
      .insert({ name: itemName, price: 0, category: '', stock: newStock, is_available: newStock !== 0 });
    return !error;
  }
}
...
export async function setUnlimitedStock(itemName) {
  ...
  if (existing) {
    const { error } = await supabase.from('menu_items')
      .update({ stock: -1, is_available: true })
      .eq('name', itemName);
    return !error;
  } else {
    const { error } = await supabase.from('menu_items')
      .insert({ name: itemName, price: 0, category: '', stock: -1, is_available: true });
    return !error;
  }
}
