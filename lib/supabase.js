...
export async function getProductStock() {
  ...
  if (menuRes.data && menuRes.data.length > 0) {
    ...
    // stock: -1 = unlimited, >=0 = exact count from POS app
    for (const item of menuRes.data) {
      map[item.name] = { name: item.name, total_in: 0, total_sold: 0, app_stock: item.stock };
    }
  ...
  return Object.values(map)
    .map(item => ({
      ...item,
      // -1 = unlimited; otherwise use the stock value synced from the POS app
