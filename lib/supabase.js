...
export async function getInventoryStatus() {
  // Fetch all purchase_items joined with their purchase, sorted newest first
  // local_id = null means added by admin via web dashboard; set = added by POS staff
  const { data } = await supabase
    .from('purchase_items')
    .select('item_name, quantity, unit_cost, purchase_id, purchases(created_at, supplier, local_id)')
  ...
  for (const row of data) {
    if (!seen.has(row.item_name)) {
      ...
      result.push({
        ...
        added_by: row.purchases?.local_id ? 'pos' : 'admin',
      });
    }
  }
