...
export async function getProductStock() {
  // Fetch total purchased per item
  const { data: purchased } = await supabase
    .from('purchase_items')
    .select('item_name, quantity');

  // Fetch total sold per item (join orders to exclude voided)
  const { data: soldData } = await supabase
    .from('order_items')
    .select('menu_item_name, quantity, orders!inner(status)');

  const sold = (soldData || []).filter(i => i.orders?.status !== 'voided');

  // Build stock map keyed by item name
  const map = {};

  for (const row of (purchased || [])) {
    const name = row.item_name;
    if (!map[name]) map[name] = { name, total_in: 0, total_sold: 0 };
    map[name].total_in += row.quantity;
  }

  for (const row of sold) {
    const name = row.menu_item_name;
    if (!map[name]) map[name] = { name, total_in: 0, total_sold: 0 };
    map[name].total_sold += row.quantity;
  }
